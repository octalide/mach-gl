...
Everything between `load` and the demo is generated by `tools/gen.py` from
the vendored registry; CI fails if the committed sources drift.
...
Paths that need a live context are exercised by the demo, not `mach test`.
